...
const Summary = ({ handleChange }: Props) => {
	...
	const getTotalPrice = (): number => {
		const planPrices = PLANS.filter(
			(el) => el.id === formCtx.formInput.plan.selected
		).reduce((acc, currentValue) => acc + currentValue.price, 0);
		const filteredAddonTitles = Object.keys(formCtx.formInput.addOns).filter(
			(key) => formCtx.formInput.addOns[`${key as keyof Inputs['addOns']}`]
		);
		const filteredAddons = ADDONS.filter((el) =>
			filteredAddonTitles.includes(el.title)
		);
		...
		return formCtx.formInput.plan.isMonthly
			? addOnPrices + planPrices
			: (addOnPrices + planPrices) * 10;
	};
		...
		<div className={`${styles.summaryContainer} section-container`}>
			...
			<div>
				<div className={styles.selectedItems}>
						...
						<div className={styles.title}>
							{formCtx.formInput.plan.selected === 1 && (
								<p>
									Arcade
									<span>
										{formCtx.formInput.plan.isMonthly
											? '(Monthly)'
											: '(Yearly)'}
									</span>
								</p>
					...
					<ul className={`${styles.addOns} flex-wrapper`}>
						{Object.entries(formCtx.formInput.addOns).map(
							([key, value], i) =>
								value && (
			...
			<div className={`${styles.total} flex-wrapper`}>
				<p>
					Total{' '}
					<span>
						{formCtx.formInput.plan.isMonthly ? '(per month)' : '(per year)'}
					</span>
				</p>
				<p className={styles.price}>
					{'+$' + getTotalPrice()}
					<span>{formCtx.formInput.plan.isMonthly ? '/mo' : '/yr'}</span>
				</p>
			</div>
		</div>
	);
